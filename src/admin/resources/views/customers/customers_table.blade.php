...
<script type="text/javascript">
    $(function() {
		...
		$(document).on('show.bs.modal', '#modal-entry', function ()
		{
			...
			if ($('#modal-entry input[name="id"]').length > 0) {
				title = 'Edit Customer';
			} else if ($('#modal-entry input[name="customer_id"]').length > 0) {
				title = 'Add Balance';
			}
        ...
        $(document).on('click', '.add-balance-row', function ()
        {
            var _this = $(this);
            var tr = _this.closest('tr');
            var table = $('.dt').DataTable();
            ...
            $.ajax({
                url: data.action_add_balance_url,
                type: 'GET',
                success: function(result)
                {
                    if (result.status || false)
                    {
                        $('#modal-entry .modal-body').empty().append(result.html);
                        $('#modal-entry .modal-body input[name="customer_id"]').attr('index', row.index());
                        $('#modal-entry .modal-body input[name="customer_id"]').attr('url', data.action_update_balance_url);
                        $('#modal-entry .btn-save').attr('data-action', 'add-balance');
                        $('#modal-entry').modal('show');
                    }
                    else
                    {
        ...
        $(document).on('click', '.btn[data-action="add-balance"]', function ()
        {
            $('#modal-entry button').prop('disabled', true);

            var formData = new FormData($('#modal-entry .modal-body .form-add-balance')[0]);

            $.ajax({
                url: $('#modal-entry .modal-body input[name="customer_id"]').attr('url'),
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(result)
                {
                    ...
                    if (result.status || false)
                    {
                        var index = $('#modal-entry .modal-body input[name="customer_id"]').attr('index');
