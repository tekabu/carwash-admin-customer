...
## POST /api/customer/rfid/{rfid}/check
```bash
curl -X POST \
...
## POST /api/customer/{customer}/points/redeem
```bash
curl -X POST \
...
## POST /api/customer/checkout/{reference}/success
```bash
curl -X POST \
  -H "Accept: application/json" \
  https://admin.example.com/api/customer/checkout/4b0ab96f-3f4e-4bb7-9f6a-1bd1c4bfb9d5/success
```
Confirms that payment succeeded for the checkout reference, marks the checkout as `DONE`, awards points (if there is a customer), and logs a new transaction.
